...
CHECKS = [
    ...
    ("duplicate records", r"The two overlap: ([\d,]+) records in the second",
     len(raw_general) + len(raw_spec) - len(inc), 0),
    ("merged corpus size", r"the combined corpus holds ([\d,]+) records", len(inc), 0),
    ...
    ("events inside the window", r"and ([\d,]+) of the placed events fall", len(in_window), 0),
    ...
    ("share holding exactly one event", r"and ([\d.]+) per cent of those that do",
    ...
    ("share of corpus placed", r"This resolved ([\d.]+) per cent of the merged corpus",
     100 * inc["pcode"].notna().mean(), 0.05),
    ("events recorded in 2026", r"retains those (\d+) events",
