...
def chapter_table(number: str) -> list[list[str]]:
...
# ---------------------------------------------------------------- 3.2 source files
print("\n-- Table 3.2, the two supplied files ------------------------------------------")
...
# ------------------------------------------------------------- Table 3.7 fold design
print("\n-- Table 3.7, rolling-origin folds --------------------------------------------")
...
rows11 = chapter_table("3.11")[1:]          # Horizon | Base | AP | Lift | R@20 | Lift@20
for row, key in zip(rows11, ("7d", "14d", "28d")):
    r = pr["horizon"][key]
    check(f"Table 3.11 {row[0]} base rate", float(row[1]), m(r, "base_rate"), tol=0.00006)
    check(f"Table 3.11 {row[0]} AP", float(row[2]), m(r, "average_precision"), tol=0.00006)
    check(f"Table 3.11 {row[0]} lift over base", float(row[3]),
          round(m(r, "ap_lift_over_base"), 1), tol=0.06)
    check(f"Table 3.11 {row[0]} R@20", float(row[4]), round(m(r, "recall_at_20"), 3), tol=0.0006)
    check(f"Table 3.11 {row[0]} lift@20", float(row[5]), round(m(r, "lift_at_20"), 1), tol=0.06)

rows12 = chapter_table("3.12")[1:]          # Delay | Base | AP | Lift | R@20 | Lift@20 | Brier
for row, key in zip(rows12, ("0w", "1w", "2w")):
    r = pr["delay"][key]
    check(f"Table 3.12 {row[0]} AP", float(row[2]), m(r, "average_precision"), tol=0.00006)
    check(f"Table 3.12 {row[0]} R@20", float(row[4]), round(m(r, "recall_at_20"), 3), tol=0.0006)
    check(f"Table 3.12 {row[0]} Brier", float(row[6]), m(r, "brier"), tol=0.00006)

u = pr["under_reporting"]
rows14 = chapter_table("3.14")[1:]          # Loss | AP | Lift | R@20 | Lift@20 | Brier
for row, key in zip(rows14, ("unweighted", "reweighted")):
    r = u[key]
    check(f"Table 3.14 {row[0][:22]} AP", float(row[1]), m(r, "average_precision"), tol=0.00006)
    check(f"Table 3.14 {row[0][:22]} R@20", float(row[3]),
...
for st, val in [("Zamfara", 0.518), ("Katsina", 0.440), ("Kaduna", 0.350), ("Borno", 0.639),
    ...
    check(f"Table 3.13 {st} detection", val, round(float(row["detection"]), 3), tol=0.0006)
...
rows15 = chapter_table("3.15")[1:]          # Band | Definition | Areas | Realised | xBase | Share
for row in rows15:
    r = rec[row[0]]
    check(f"Table 3.15 {row[0]} areas/week", float(row[2]),
          round(r["areas_per_week"], 1), tol=0.06)
    check(f"Table 3.15 {row[0]} realised rate", float(row[3]),
          round(r["realised_rate"], 3), tol=0.0006)
    check(f"Table 3.15 {row[0]} times base", float(row[4]), round(r["times_base"], 1), tol=0.06)
    check(f"Table 3.15 {row[0]} share of events %", float(row[5].rstrip("%")),
